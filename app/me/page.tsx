 ...
 * `points` string. Used for every notched-outline frame on this page --
 ...
 * via ResizeObserver -- the exact technique the profile card's frame uses
 * (nested notchedRectPoints polygons, outer to inner), generalized so any
 * number of bands can be dropped over a button/panel to fake the same
 * layered-depth border. `colors` goes outer -> inner; the last color becomes
 * the element's visible fill wherever nothing else covers it, same as the
 * profile card has no separate CSS background of its own. Rendered behind
 * the element's normal content via zIndex:-1 (the parent must be
 * position:relative so that negative z-index still resolves within its own
 * stacking context instead of falling behind the whole page). */
function NotchedFrame({ colors, step = 4, ringWidth = 4 }: { colors: string[]; step?: number; ringWidth?: number }) {
