...
usual "fit the tallest frame flush to the cell" auto-scale -- matches the
character-height fill of officer-sprite.png's other 7 directions (all
generated together in one combined call there, so they share a scale;
this sheet is scaled independently since it's a different source image, so
without pinning to the same ratio it would come out a different size next
to up/left/right/etc). Run manually
