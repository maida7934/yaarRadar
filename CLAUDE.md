...
repo (`walk-backend`, local sibling folder `yaarRadar/`), which itself sits in
front of Supabase (Postgres + Auth + Realtime). This document is the complete
contract between the two — read it fully before writing any code that touches
auth, friends, locations, or the database in any way.
